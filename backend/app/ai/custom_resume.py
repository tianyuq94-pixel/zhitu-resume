...
from app.ai.prompts.custom_resume import SYSTEM_PROMPT, build_user_prompt
from app.core.config import Settings, get_settings
from app.schemas.custom_resume import GeneratedCustomResumeResult, validate_generated_custom_resume


@dataclass(frozen=True)
class GeneratedCustomResume:
    result: GeneratedCustomResumeResult
    ...
    *,
    ...
    for attempt in range(active_settings.deepseek_max_attempts):
        try:
            ...
            result = GeneratedCustomResumeResult.model_validate(completion.data)
            validate_generated_custom_resume(result, resume_text)
            return GeneratedCustomResume(
                result=result,
            ...
            last_error = AIServiceError(
                "AI_RESPONSE_INVALID",
                "DeepSeek response did not match the custom resume schema",
                retryable=True,
            )
    ...
    raise last_error or AIServiceError("AI_RESPONSE_INVALID", "Unable to generate custom resume")
